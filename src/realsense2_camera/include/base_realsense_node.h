/*
创建一个 realsense 相机的基础节点类及其一些必要的功能实现。
① image_publisher
② PipelineSyncer   ： 
③ SyncedImuPublisher
④ BaseRealSenseNode
...
// 导入cv_bridge库， cv_bridge.h 最后支持的版本是 Humble。
...
// 导入 diagnostic_updater 库 用于诊断信息的发布和管理
#include <diagnostic_updater/diagnostic_updater.hpp>
#include <diagnostic_updater/publisher.hpp>

// 导入 realsense2_camera_msgs 库
// ① 惯性测量单元（IMU）数据
// ② 相机的外参
// ③ 相机的元数据（曝光、增益、白平衡）
// ④ 图片 RGB-D 数据
...
// 导入 realsense2 数据处理 包
#include <librealsense2/hpp/rs_processing.hpp>

// 导入 RealSense 的“高级模式”接口
#include <librealsense2/rs_advanced_mode.hpp>

// 图像编码格式的定义
#include <sensor_msgs/image_encodings.hpp>

// ros2 中的数据格式 sensor_msgs
...
{
    // 定义一种类型 stream_index_pair ， (rs2_stream，int）
    ...
    */
    ...
    // 处理帧的异步同的 asynchronous_syncer ，确保各帧数据的时间对齐
    ...
    class SyncedImuPublisher
    {
        public:
            ...
            ~SyncedImuPublisher();
            void Pause();   // 暂停发送消息。此后所有消息都将保存在队列中。
            void Resume();  // 发送所有未发送的消息，并允许发送未来的消息。
            void Publish(sensor_msgs::msg::Imu msg);  // 发送或暂存消息。
