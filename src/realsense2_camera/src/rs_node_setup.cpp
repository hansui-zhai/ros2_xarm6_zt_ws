                                                            ...
                                                            )))
            {
                if (!wanted_profiles.empty())
                {
                    ...
                    if (sensor->rs2::sensor::is<rs2::depth_sensor>())
                    {
                        ...
                        ROS_INFO_STREAM("[DEBUG!]Depth Scale Meters: " << _depth_scale_meters);
                        ROS_INFO_STREAM("[DEBUG!]_align_depth_filter: " << _align_depth_filter->is_enabled());
                        ROS_INFO_STREAM("[DEBUG!]_colorizer_filter: " << _colorizer_filter->is_enabled());
                        ROS_INFO_STREAM("[DEBUG!]_clipping_distance: " <<  _clipping_distance);
                    }
                }
            }
        }
